...
- New AWS resources: instance class, RDS size, S3 lifecycle policy,
  Lambda memory/timeout. Flag any new resource without a tag for cost-center.
- Network egress: cross-AZ or cross-region traffic introduced.
- Third-party APIs: per-call cost × expected QPS × retention.
- Model usage: respects `config/model-routing.md`. Flag any code path that
  defaults to Opus when Sonnet would suffice.
